...
fn test_nil_vectors() {
    ...
    for test in &vectors.nil {
        ...
        assert!(matches!(value.typed(), Typed::Nil), "Expected nil for bytes: {}", test.bytes);
    }
}
...
fn test_bit_vectors() {
    ...
    for test in &vectors.bit {
        ...
        let value = Value::new(&bytes, 0).expect("Failed to create Value");
        match value.typed() {
            Typed::Bool(b) => assert_eq!(b, test.parsed.value, "Wrong value for: {}", test.bytes),
...
fn test_bin_vectors() {
    ...
    for test in &vectors.bin {
        ...
        match value.typed() {
            Typed::Bytes(b) => assert_eq!(
                b, expected_bytes.as_slice(),
...
fn test_arr_vectors() {
    ...
    for test in &vectors.arr {
        let bytes = hex_to_bytes(&test.bytes);
        let value = Value::new(&bytes, 0).expect("Failed to create Value");
        let arr = match value.typed() {
            Typed::Arr(arr) => arr,
            _ => panic!("Expected Arr for: {}", test.bytes),
        };
        assert_eq!(
            arr.is_root(),
            test.parsed.is_root,
...
fn test_map_vectors() {
    ...
    for test in &vectors.map {
        let bytes = hex_to_bytes(&test.bytes);
        let value = Value::new(&bytes, 0).expect("Failed to create Value");
        let map = match value.typed() {
            Typed::Map(map) => map,
            _ => panic!("Expected Map for: {}", test.bytes),
        };
        assert_eq!(
            map.is_branch(),
            test.parsed.is_branch,
        ...
        if test.parsed.is_branch {
            ...
            for (i, entry) in test.parsed.entries.iter().enumerate() {
                if let MapEntry::Branch(addr) = entry {
                    let actual = map.get_child_addr(i);
                    assert_eq!(
                        actual,
                        Some(*addr),
                        "Wrong child {} for: {}",
            ...
            // Leaf node - check key/value pairs
            let kv_pairs: Vec<_> = map.kv_pairs().collect();
            assert_eq!(
                kv_pairs.len(),
