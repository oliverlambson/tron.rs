...
fn arr_set_internal(
    ...
    let new_child_addr =
        arr_set_internal(doc, child_addr, index, value_addr, false, None)?;
    ...
    if required > current_shift {
        ...
        let (old_shift, old_bitmap, old_entries, old_is_leaf) = {
            ...
            let entries: Vec<u32> = (0..old_root.entry_count())
                ...
                .collect();
            (old_root.shift(), old_root.bitmap(), entries, old_root.is_leaf())
        };
...
fn arr_append_internal(
    ...
    let new_child_addr = arr_append_internal(doc, child_addr, index, value_addr, false, new_length)?;
...
impl<'a> CachedArray<'a> {
    ...
    pub fn get(&self, index: u32) -> Result<Option<u32>> {
        ...
        // Check cache
        {
            ...
            if let Some(Some(leaf_addr)) = cache.get(chunk_index) {
                return self.get_from_leaf(*leaf_addr, index);
            }
        }
        ...
        self.get_from_leaf(leaf_addr, index)
    }
    ...
    fn get_from_leaf(&self, leaf_addr: u32, index: u32) -> Result<Option<u32>> {
...
mod tests {
    ...
    fn test_arr_append_multiple() {
        ...
        for (i, expected) in [10i64, 20, 30].iter().enumerate() {
            ...
            assert!(result.is_some(), "index {} not found", i);
    ...
    fn test_arr_set() {
        ...
        let result = arr_get(doc.as_bytes(), doc.root_addr(), 0).unwrap();
        let value = crate::value::Value::new(doc.as_bytes(), result.unwrap()).unwrap();
        assert!(matches!(value.typed(), Typed::I64(1)));
    }
    ...
    fn test_arr_multi_level() {
        ...
        for (i, expected) in values.iter().enumerate() {
            ...
            assert!(result.is_some(), "index {} not found", i);
            let value = crate::value::Value::new(doc.as_bytes(), result.unwrap()).unwrap();
            assert!(matches!(value.typed(), Typed::I64(n) if n == *expected), "index {} wrong value", i);
        }
    }
