...
use crate::tag::{minimal_node_len_encoding, minimal_uint_encoding, Tag};
...
impl Encode for ArrLeaf<'_> {
    fn encode(&self) -> Vec<u8> {
        encode_arr_leaf(self.is_root, self.shift, self.bitmap, self.length, self.entries)
    }
}
...
impl Encode for ArrBranch<'_> {
    fn encode(&self) -> Vec<u8> {
        encode_arr_branch(self.is_root, self.shift, self.bitmap, self.length, self.children)
    }
}
...
mod tests {
    ...
    fn test_encode_i64() {
        ...
        assert_eq!(encoded, [0x02, 0xD2, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
    }
